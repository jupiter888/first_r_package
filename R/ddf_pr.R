#' DDF on Precipitation data
#'
#' @param dt Dataset including p(pr kg/s, data, SID(files names))
#'
#' @return Processed dataset with new cols
#' @export
#'
#' @examples
ddf_pr<-function(dt){
  ...
  dtm<-melt(dt, id.vars=c('year','SID'))
  dt<-dtm[,.(depth=max(value, na.rm=TRUE)), by=.(year, variable, SID)]
